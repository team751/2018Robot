...
public class CheesyDrive {
	 ...
	 *
	 * @param throttle
	 *            Move value: +1 is full speed forwards, -1 is full speed
	 *            backwards
	 * @param wheel
	 *            Rotation value: -1 is full left, +1 is full right
	 * @param isQuickTurn
	 *            If quick turn mode should be enabled
    ...
    // http://www.wolframalpha.com/input/?i=sin%28pi%2F2+*+0.8+*+x%29+%2F+sin%28pi%2F2+*+0.8%29
		...
		if (isQuickTurn) {
			...
			if (Math.abs(linear_power) < 0.2) {
				double alpha = .1;
				quickStopAccumulator = (1 - alpha) * quickStopAccumulator + alpha * pwmLimit(wheel) * 5;
			}
			overPower = 1.0;
			angular_power = wheel;
		} else {
		...
		// return new MotorOutputs(left_pwm*(rdedistance/ldedistance),
		// right_pwm);
		// } else {
		// return new MotorOutputs(left_pwm,
		// right_pwm*(ldedistance/rdedistance));
		// }
