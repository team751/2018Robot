...
int sendRepeat = 3; // Variable that denotes how many times the Arduino will send the queried information if it does not receive the confirmation token.
...
void sendData() {
  ...
  while (count++ < sendRepeat && !Serial.readString().equals(confirmationToken + separator + String(requestID))) { // While the RoboRIO hasn't sent the right verification token.
    ...
    Serial.print("-");
    Serial.print(getOrientation(debug));
    Serial.print("-");
    Serial.print(getDistanceTraveled(debug));
    Serial.print("-");
    Serial.println(getVelocity(debug));
    delay(2000);
  }
}

void recalibrate(boolean debug) {
  if (debug) {
...
float getDistanceTraveled(boolean debug) {

}

float getVelocity(boolean debug) {
