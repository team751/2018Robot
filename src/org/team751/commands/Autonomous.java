...
public class Autonomous extends Command {
	...
	// 0.925 in Bellarmine
	private static double ratio = 0.225/0.185;
	private static double rightSpeed = -leftSpeed * ratio;
	...
	protected void leftGoLeft(double time) {
		...
		if (time <= 1.12) {
			...
			if (time > 2.52 && totalCurrent > currentLimit) {
				Robot.crushed = true;
			}
	...
	protected void rightGoRight(double time) {
		// forward
		if (time <= 3.17) {
			if (totalCurrent > currentLimit + 20) {
				Robot.crushed = true;
			}
			Robot.drivetrain.setLeftSpeed(leftSpeed);
			Robot.drivetrain.setRightSpeed(rightSpeed);
			...
			turnCCW();
		} else if (time > 6.17 && time < 10) { // turn right
			if (totalCurrent > currentLimit) {
				Robot.crushed = true;
			}
			Robot.drivetrain.setLeftSpeed(leftSpeed * 0.25);
			Robot.drivetrain.setRightSpeed(rightSpeed * 0.25);
		}
	}
